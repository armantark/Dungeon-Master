...
- `src/dungeon_master/transport/stream_runtime.py`: detached stream runtime used by FastAPI streaming routes. `SessionRegistry` owns request tokens and retained session lifecycle; `StreamRuntime` owns background execution, NDJSON production, replay/live-tail subscribers, save-scope validation, and shutdown. The former `stream_session.py` duplicate is deleted.
...
- `src/dungeon_master/config/`: typed runtime/model config package. `app.py` defines `AppConfig`, `LLMConfig`, `TaskProfile`, and `LLMProfiles`; desktop-beta follow-up widens that seam with a dedicated `LLMCredentialsStore`, provider resolution order (stored BYOK key -> env -> unavailable), and a separate credentials path alongside `runtime_settings.json`. `README.md` explains the env surface and why narrator-facing knobs stay separate from low-temperature structured-call profiles.
...
- `src/dungeon_master/inventory_updater.py`: focused backend-only structured updater for durable carried/worn item canon after narration. It extracts only bounded `add_item | remove_item` ops from committed prose, then delegates the actual mutation to the existing Cairn acquisition/drop engine so slots/tags/equipped state/burden recomputation stay centralized in `src/dungeon_master/cairn.py`.
...
- `src/dungeon_master/service.py`: application facade for gameplay use cases. It coordinates planning, deterministic mechanics, narration, persistence, cancellation, and save binding while delegating the successful-turn tail to `application/turn_commit.py`. Every completed narration now receives one bounded post-narration continuity reconciliation opportunity; the deleted classifier no longer adds a separate model call or pre-narration branch. Streaming session ownership is delegated to `transport/stream_runtime.py`, and campaign-generation details are delegated to the `generation/` package.
- FastAPI streamed routes no longer let the HTTP response own model progress. `src/dungeon_master/api.py` delegates each streamed request to `StreamRuntime`; `GET /api/requests/{request_id}/stream` reattaches to the retained buffer, replays prior deltas, and rejects mismatched save scopes with `409`.
...
- `src/dungeon_master/settings.py`: thin compatibility wrapper around `AppConfig.from_env()` for `state_path`, `runtime_settings_path`, and the new desktop-credential path.

Frontend:

- `web/src/main.ts`, `web/src/App.svelte`: entry + setup-aware layout (CharacterSetup before campaign start; StatusStrip → CharacterFolio + ChatFeed → Composer → Inspector once active). Desktop-beta follow-up adds an app-level runtime bootstrap gate before save-library bootstrap so packaged builds can prompt for a provider key when no `.env`/stored key exists.
- `web/src/lib/api.ts`: thin fetch wrapper, now also exposing character template/draft/finalize/start and message-regenerate endpoints. The streaming Phase 2 frontend pass adds `reattachStream(requestId, handlers, signal?)` against the new backend `GET /api/requests/{id}/stream` route; it shares the same NDJSON transport via `consumeStream` and just toggles its new `method: "GET" | "POST"` option. The newer `/ask` split adds `previewYesNo(question, likelihood, signal?)` against `POST /api/oracle/yes-no/preview`, while the legacy `askYesNo(...)` call remains available for the still-canonical committed oracle route. Desktop-beta follow-up makes the API base runtime-configurable (`/api` in browser dev, injected local sidecar base in Tauri) and adds `updateLlmCredentials(provider, api_key)`.
...
- `web/src/components/`: `StatusStrip`, `SystemMenu`, `SaveLibrary`, `CharacterSetup`, `CharacterTemplateCard`, `CharacterEditor`, `LoadingPanel`, `CharacterFolio`, `CairnReadout`, `ChatFeed`, `ChatMessage`, `MessageActions`, `MechanicalReceipt`, `CombatTracker`, `Composer`, `EndBanner`, `Inspector`, `ChaosDial`, `Drawer`, `ThreadsPanel`, `NPCsPanel`, and `DirectivesEditor`. `CairnReadout.svelte` is the shared read-only mechanics surface used by `CharacterFolio` (active play) and `CharacterEditor` (post-backfill draft preview). F-05 layered an ambush cue across `StatusStrip` (kicker swap + tint), `CombatTracker` (rust-blood `Ambush` flag), and `MechanicalReceipt` (strip tag swap + rust-blood rail + Cairn `Initiative` row) without inventing new components. H-01/H-02 extend those existing components rather than introducing new navigation chrome: `NPCsPanel.svelte` now renders descriptor-visible figures by `player_label` with a `known by sign` pip, `MechanicalReceipt.svelte` renders compact continuity pills for touched threads / visible figures, `Drawer.svelte` accepts a `reopenToken` one-shot reopen nudge, and `Inspector.svelte` / `ChatMessage.svelte` / `ChatFeed.svelte` now thread enough current-state context through the receipt path for those pills to deep-link into the existing Inspector surfaces. The `/ask` split reuses those same OOC chat surfaces rather than adding a new component: `ChatFeed.svelte` and `history.ts` now treat `oracle_preview` notes as verdigris OOC cards/searchable ephemeral notes alongside `explanation`. F-12 added two new components: `SystemMenu.svelte` (top-right hamburger dropdown with `Save library` / `Switch save` / `Begin a new campaign`, mounted on both the play `StatusStrip` and the skeleton strip used during character creation) and `SaveLibrary.svelte` (parchment-deckle card-grid splash that doubles as the empty-shelf frontispiece and the mid-session selector). Desktop-beta follow-up adds `CredentialSetupModal.svelte`, a dedicated BYOK overlay used both for first-run gating and later key updates from the settings modal. Removed in the chat-first pivot: `SceneCard`, `OracleConsole`, `PlayerCommand`, `ActionLog`, `DiceTumbler` (the dice now live inline in `MechanicalReceipt`).
...
- Current simplification verification: repository-wide Ruff, mypy, and pytest pass; frontend `svelte-check`, 342 Vitest cases, and the production Vite build pass. An isolated PinchTab smoke switched between fixture saves without state bleed and committed a deterministic oracle event after switching back.
...
- `cd web && npm run sidecar:build` (build the PyInstaller backend sidecar expected by Tauri)
- `cd web && npm run tauri:dev` / `npm run tauri:build` (desktop beta; requires Rust installed on the build machine)
...
- Dynamic thread, NPC, and inventory continuity updates do not add new API routes. They flow through the existing oracle/turn endpoints because `GameService` now reconciles them inside the ordinary turn pipeline, and the full returned `GameState` already includes the updated canon.
