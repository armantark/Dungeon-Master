...
Thread continuity now follows a two-phase pattern. The old pre-narration seam still exists for direct oracle/admin paths, but natural-language turns now treat **post-narration reconciliation as the primary durable-canon pass**. After Python resolves the deterministic outcome and narration is committed, `GameService` can run the focused backend thread updater against the actual prose, returning a validated bounded batch of `create | update | resolve` ops that Python applies to canonical `GameState.threads`. This keeps narration prose-only while letting long-running threads evolve from committed fiction rather than speculative pre-prose guesses. Invalid/duplicate ops collapse safely to no-ops rather than blocking the main turn.
...
One more orchestration seam now sits in front of both continuity updaters: `src/dungeon_master/continuity_classifier.py`. Rather than deterministically keyword-gating continuity work or always paying for both model calls, `GameService` first makes one tiny model request for a single scope token: `none | threads | npcs | both`. The classifier sees only the resolved turn, the current scene, and very small continuity snapshots, and it is instructed to be conservative: if unsure, choose `both`; if the call fails or the output is malformed, the service also falls back to `both`. The newer follow-up sharpens the post-narration rule: `none` is now reserved for genuinely non-durable prose, while anything that appears to settle an ongoing matter or pin down a recurring person should choose the safer updater scope. This preserves the project's rule "use the model for interpretation, not regex heuristics" while still letting obviously local/self-contained turns skip unnecessary continuity-update cost. Crucially, the service still normalizes `referenced_thread_ids` and visible-only `referenced_npc_ids` even when one or both expensive updater calls are skipped, so receipt/navigation invariants do not depend on the classifier deciding to run.
