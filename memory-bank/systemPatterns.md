...
The HTTP surface is intentionally thin. `src/dungeon_master/transport/http/asgi.py` is a stable import facade; `transport/http/app.py` assembles the app and `transport/http/routes/` exposes one route per application operation. Mutating routes return the entire `GameState`. That keeps the frontend reconciler trivial (last response wins) and matches the single-writer single-user assumption. Authentication is omitted on purpose because the server binds to `127.0.0.1` and is meant to run on the user's own machine.
...
- **Backend turn planner** (`src/dungeon_master/llm/planning/`, exported through its package `__init__.py`): bare natural-language chat goes here. It asks the model for a bounded **typed action plan**: 1-3 ordered steps drawn from a fixed vocabulary (`yes_no`, `random_event`, `scene_check`, `save`, `attack`, `harm`, `recovery`, `equip`, plus non-combat prep/state-adjacent steps like `inspect_inventory`, `search_scene`, `use_item`, `drop_item`, and `narrate`). The returned `route` remains a summary label for compatibility, while `application/turn_plan_execution.py` executes the ordered plan. Planner output is validated through Pydantic and receives one schema-aware repair attempt. If both attempts fail, it degrades to a one-step `player_action`/`narrate` plan so it cannot invent deterministic mechanics.
- **Backend `GameService`** (`src/dungeon_master/application/game_service.py`): receives a typed call (`ask_yes_no`, `random_event`, `scene_check`, `submit_action`, etc.) and runs the deterministic mechanics in the right order, never trusting the LLM with state mutation. The router fallback is intentionally safe rather than silent mechanical inference: a failed structured plan can still produce prose for the player's exact text, but it cannot invent a save, attack, item transfer, or other deterministic operation.
...
The current code no longer hardcodes oracle content tables in `src/dungeon_master/mechanics/oracle.py`. New campaigns ask the model to generate oracle word banks, then persist those banks in `GameState.oracle_tables`; the deterministic oracle rolls over the persisted generated tables.
...
Thread continuity has one durability boundary: **post-narration reconciliation**. After Python resolves the deterministic outcome and narration is committed, `TurnCommitter` gives the focused thread updater one bounded opportunity to return validated `create | update | resolve` operations that Python applies to canonical `GameState.threads`. A narration with no durable thread change produces an empty operation batch. There is no speculative pre-prose continuity lane.
...
NPC continuity follows the same bounded-updater pattern as threads, but with one extra invariant: recurring people should not vanish from canon just because they leave the active cast. For natural-language turns the updater now primarily runs after narration, sees the resolved `OracleOutcome`, player input, executed backend steps, committed prose, current NPC list, and bounded NPC-oriented memory context, and returns a validated bounded batch of `create | update | retire` ops. Python applies those ops to canonical `GameState.npcs`, and touched ids are merged back onto the outcome via plural `referenced_npc_ids` plus a primary `referenced_npc_id` for compatibility. NPCs are never deleted here; instead, `NPC.status` becomes `active` or `retired`. The prompt contract now also explicitly says not to collapse two context-distinguished people into one record just because they share a broad category. That keeps recurring-cast continuity available to the memory sidecar and the inspector while still letting the active cast shrink visually. "Promote" is intentionally not a separate op kind — it is just an `update` that changes `role` and/or `disposition`.

Continuity scope is no longer classified by a separate model call. The thread and NPC updaters each receive the committed narration and may return an empty bounded operation batch. This removes one judge-like decision point while preserving typed model proposals, Python validation, visible-only NPC references, and a single canonical commit boundary.

Inventory continuity now follows the same separation-of-concerns rule rather than letting prose-only items drift out of canon. `src/dungeon_master/application/updates/inventory.py` is a focused post-narration structured updater that extracts only durable carried/worn item changes (`add_item | remove_item`) from committed prose. It does **not** invent item mechanics itself; instead it reuses the existing `CairnEngine.acquire_items(...)` / `drop_item(...)` path so tags, slots, equipped state, and burden recomputation still live in one deterministic engine. The prompt is intentionally general-purpose: ordinary clothing, accessories, tools, documents, and drinks all qualify when the narration makes them part of the actor's ongoing personal carry, while scenery and temporary props do not.
...
Narrative threat warnings stay in the narrator prompt layer, not in canonical mechanics or the UI. `NarrativeEngine` now computes a private `DIEGETIC_THREAT_APPRAISAL` from the active encounter and active party: foe HP/STR/armor/damage/threat tier versus party HP/STR/armor/conditions. If that score says the party is outmatched, the narrator is instructed to telegraph the danger through fiction only: size, reach, bad footing, exhaustion, ally hesitation, and prep/escape vectors. It must not say "level", "not intended", "mechanically too hard", or otherwise break character. This keeps difficulty communication aligned with the project boundary: Python supplies deterministic state and risk context; the LLM turns it into prose without changing rolls, HP, danger profile, or canonical encounter state. Scene interruptions follow the same boundary: `SCENE_INTERRUPTION_CONSTRAINTS` tells the narrator that an interrupted scene check licenses an obstacle before the expected scene, not arbitrary physical reappearance by older threats. Stalker enemies are allowed only when canonical state marks an active pursuit and the foe can plausibly reach the scene; otherwise older boss/quest-threat memory should manifest indirectly through traces, damage, minions, rumors, witnesses, blocked routes, or time pressure.
...
- in the current backend implementation, the above is realized as a separate `CairnEngine` (`src/dungeon_master/mechanics/engine.py`) that sits beside `OracleEngine`, not inside it
...
policy prose. `src/dungeon_master/llm/prompt_fragments.py` owns reusable blocks
...
- `src/dungeon_master/llm/narration/engine.py` exposes both buffered generation (`generate_result`) and generator-based streaming (`iter_stream`) using a shared `CompletionDelta` / `NarrativeResult` contract.
- `src/dungeon_master/generation/__init__.py` now mirrors that pattern for non-play setup flows. Character templates, quiz generation, draft generation, quizzed draft generation, and campaign world generation each have both buffered `*_result` entry points and `iter_*` generator entry points, so setup no longer needs a separate bespoke transport.
- `src/dungeon_master/application/game_service.py` lifts those into app-level result + stream methods and persists thinking where canon exists. If the stream resolves to a `GameState`, the final thinking is attached to a persisted `GameEvent`; if the stream resolves to a setup artifact (templates / quiz / draft), the final thinking is returned in the terminal API payload because there is no canonical active campaign state yet.
- `src/dungeon_master/transport/http/asgi.py` standardizes the wire format as **NDJSON** (`Content-Type: application/x-ndjson`, one JSON object per `\n`). The lifecycle is `meta` → `thinking_delta*` → `content_delta*` → exactly one terminal `final_state` / `final_payload` / `error`. SSE was rejected because every streamed endpoint is a POST with a JSON body (EventSource is GET-only) and the frontend was already going to need a custom `fetch` + `ReadableStream` parser; NDJSON is simpler than SSE once you're parsing manually anyway.
...
- Streaming requests now also have a **process-local cancellation contract**:
  - `src/dungeon_master/transport/stream_runtime.py` owns a `SessionRegistry` keyed by `request_id`; it creates cooperative `CancellationToken`s from `src/dungeon_master/application/cancellation.py`.
  - `src/dungeon_master/transport/http/asgi.py` registers a token before each streamed response, emits the same `request_id` in the leading `meta` event, exposes `POST /api/requests/{request_id}/cancel`, and silently closes the NDJSON stream on `RequestCancelledError` rather than serializing cancellation as an `error`.
  - `src/dungeon_master/llm/narration/engine.py`, `llm/planning/`, `generation/`, and `mechanics/` thread the token into LiteLLM request objects so route classification, setup generation, encounter seeding, and narrative streaming can all stop cooperatively.
  - `src/dungeon_master/application/game_service.py` treats streamed mutations as **queued until commit**: streamed player/oracle/system events are held in memory, streamed turn checkpoints are deferred until the final commit path, and `_persist_streamed_state` re-checks cancellation before writing anything. The intended semantics are discard-only: a cancelled turn leaves canonical state/event history unchanged.
...
- **`Inspector.svelte`** (slide-in right drawer, default closed): compact chaos control row, collapsed threads, NPCs, notes editor, transcript search, full oracle history (with text + kind filters), reset campaign button. All sections are collapsed by default to prevent the unusable nested-scroll/tall-dashboard behavior the user rejected. The inspector is structured as a 3-row flex column — `<header>` / `<div class="body">` (the only scrollable region, with `overflow-y: auto` and `scrollbar-gutter: stable`) / `<footer class="end">` — so the lifecycle buttons sit beneath the scroll surface as a fixed band rather than sticky-overlaying the bottom drawer. Drawer sections (`web/src/features/inspector/Drawer.svelte`) pin `flex-shrink: 0` so they keep their flap's intrinsic `min-height: 3.55rem` even when seven of them stack inside a short viewport — without this, the parent flex column would silently squash flap labels and chevrons under each other. Drawer bodies and the inspector body both reserve `scrollbar-gutter: stable` and apply `overflow-wrap: anywhere` as a defensive horizontal-overflow fallback. Long prose (NPC dispositions, thread stakes, setting/player notes preview, Cairn build notes) wraps freely without `-webkit-line-clamp` because the drawer's internal scroll is the right escape hatch when content is genuinely long; the previous clamps silently dropped the back of every long string and forced the player to hover for a `title`-tooltip just to read past the cap.
...
9. **`inset` box-shadow on the button is covered by a `::before` pseudo at `z-index: -1`.** Painting order inside the isolated stacking context renders the element's own background and `inset` box-shadow in step 1, then negative z-index descendants in step 2 — so any `inset` bevel you put on the button itself gets silently covered by the cast-iron texture pseudo and the button reads as having "no bevel at all." Keep only outset drop shadows on the button. The bevel itself must live on the `::before` (above the texture, below the text) or there'll be no bevel at all.

10. **Bevel must read as sculpted iron, not a gold/black pixel ring.** A simple `inset box-shadow` on small button sizes reads as a uniform border line rather than as a chamfered edge because the blur/spread naturally rounds the inner corners, looking like a "bullnose" or "fillet". Using flat gradient strips hides the texture underneath. The correct pattern for a perfect 45-degree mitered chamfer is to use a semi-transparent `border` on a `::before` pseudo-element that is `inset: 0` and has the cast-iron `background-image`. Because `background-clip` defaults to `border-box`, the texture renders underneath the border, and the `rgba()` border colors perfectly tint the texture to create lit and shadowed faces that meet at precise 45-degree diagonal corners. This ensures the bevel looks like the same physical material.

11. **Background-size `cover` destroys texture scale on buttons.** Using `background-size: cover` on a button with a square texture (like cast iron) scales the image so its width matches the button's width. On a small 100x30 button, a 1200px texture shrinks to 100px — a 12x reduction that turns the grain into invisible subpixel noise. It also makes horizontal panning via `background-position` impossible. Always use a fixed large pixel size (e.g. `background-size: 800px 800px`) for button textures so the grain remains at a consistent, high-fidelity scale across all UI elements and can be randomized on both axes.
...
- Frontend rendering of Cairn state lives in a small dedicated module:
  ...
  - `web/src/features/combat/CairnReadout.svelte` — read-only stat / burden / statuses / skills / abilities / (optional) build-notes block. Surface-agnostic so it can sit inside the folio's iron rail and above the editor's parchment surface.
...
### One post-narration commit boundary

- Python resolves the typed plan and mechanics before prose.
- Completed prose is passed once to `TurnCommitter`, which applies character effects, inventory proposals, NPC disclosure, thread/NPC continuity proposals, visible-reference normalization, and committed-turn memory construction.
- Thread and NPC proposals can be generated concurrently, but Python applies them in a fixed order before the canonical save commit.
- There is no pre-narration continuity classifier. A no-op proposal is the cheap, explicit outcome when narration establishes no durable change.

### Detached stream ownership

- `SessionRegistry` is the sole owner of request tokens and retained stream-session lifecycle.
- `StreamRuntime` owns background generator execution and NDJSON publication. FastAPI routes only start, attach, cancel, or validate a session.
- A browser disconnect removes a subscriber, not the producer. Explicit cancellation still prevents the in-flight canonical commit.

### Frontend terminal ownership

- `consumeStream` parses and returns one terminal result. It never applies terminal state through callbacks.
- The store converts and applies that result once through `store/stream-runner.ts`.
- Save selection publishes the selected id and fetched `GameState` together so the store cannot display state under the wrong save identity.
