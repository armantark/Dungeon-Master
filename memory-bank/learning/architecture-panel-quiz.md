...
| 7. Full-state replacement rationale (2026-08-20) | Partial: correctly located replacement on the frontend and identified snapshot transfer, parsing, memory, and weaker-device or connection costs as likely scaling limits. | The client does not rebuild the whole DOM on every assignment; Svelte updates affected render output. Defend the current contract as simple server-authoritative convergence that avoids patch-order and client-merge drift, then name measurable thresholds and a migration that preserves server authority. |
