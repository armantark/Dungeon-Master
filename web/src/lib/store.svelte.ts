...
interface StreamingState {
  ...
  resuming: boolean;
  // Ordered backend checklist. Empty while no stages have been
  // observed; populated by the bootstrap frame the backend emits at
  // stream open and updated in place by subsequent `stage` events
  // that flip status to active/done/skipped. Hybrid continuity can
  // append a late reconciliation stage after prose has already
  // started streaming, so this list intentionally models the whole
  // visible pipeline rather than only the pre-narration slice.
