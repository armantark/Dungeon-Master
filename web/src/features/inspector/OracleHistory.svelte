<script lang="ts">
  ...
  let kinds: Set<OracleKind> = $state(new Set<OracleKind>());
...
<div class="search">
  <input type="search" class="search__input" placeholder="Filter rolls — names, scars, weapons…" aria-label="Filter oracle history" bind:value={query} />
  {#if filtersActive}
    <button type="button" class="ghost search__clear" onclick={clearFilters} aria-label="Clear filters">Clear</button>
  {/if}
</div>

<div class="kind-pills" role="group" aria-label="Filter by roll kind">
  {#each KIND_FILTER_OPTIONS as option (option.kind)}
    <button type="button" class="kind-pill pixel" class:kind-pill--on={kinds.has(option.kind)} aria-pressed={kinds.has(option.kind)} onclick={() => toggleKind(option.kind)}>{option.label}</button>
  {/each}
</div>
    ...
    {#each filteredHistory as outcome (outcome.id)}
      <li class="history__row">
        ...
        {#if findNarrativeEventForOracle(gs, outcome.id) !== null}
          <button type="button" class="ghost history__jump pixel" onclick={() => jump(outcome.id)} title="Show this roll's narration in the chat">Show in chat</button>
        {/if}
      </li>
    {/each}
  </ul>
...
<style>
  ...
  .kind-pills { display: flex; flex-wrap: wrap; gap: 0.3rem; margin-bottom: 0.45rem; }
  .kind-pill { padding: 0.28rem 0.5rem; font-size: 0.66rem; letter-spacing: 0.06em; text-transform: uppercase; color: color-mix(in oklab, var(--paper-shadow) 92%, transparent); background: rgba(0, 0, 0, 0.25); border: 1px solid color-mix(in oklab, var(--gold-tarnished) 30%, transparent); border-radius: 2px; cursor: pointer; }
  .kind-pill:hover { color: var(--paper-bone); border-color: var(--gold-tarnished); }
  .kind-pill:focus-visible { outline: 2px solid var(--gold-bright); outline-offset: 1px; }
  .kind-pill--on { color: var(--gold-bright); border-color: var(--gold-bright); background: color-mix(in oklab, var(--gold-bright) 10%, transparent); box-shadow: 0 0 0 1px color-mix(in oklab, var(--gold-bright) 18%, transparent) inset; }
  .history { list-style: none; padding: 0; margin: 0; display: flex; flex-direction: column; gap: 0.4rem; }
  .history__row { display: flex; flex-direction: column; gap: 0.25rem; }
  .history__jump { align-self: flex-start; padding: 0.28rem 0.55rem; font-size: 0.66rem; letter-spacing: 0.06em; text-transform: uppercase; }
</style>
